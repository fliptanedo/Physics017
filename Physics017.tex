...
\end{align}
The set of all possible vectors that can be formed this way is called the \textbf{vector space} \emph{spanned by}  $\vec{v}_1, \cdots, \vec{v}_N$. When we say that linear combinations of vectors are also vectors, what we mean is that they are \emph{also vectors in the vector space}. We write vector spaces with a capital letter, say $V$, and write that a vector $\vec{v}$ is part of the vector space by writing
\begin{align}
    \vec{v} \in V \ .
\end{align}
...
% Observation that  \eqref{eq:linear:combination:looks:like:basis} looks like our definition

% def of dimension
% Vectors and bases
%  Use Fibonnacci eg from above
...
The idea of a vector space lets us sharpen some of our language. A \textbf{matrix}\index{matrix} is a function that takes vectors and spits out other vectors. We say that these are maps from $V\to V$. Equivalently, the matrix also takes row vectors and spits out row vectors, so they are also maps from $V^* \to V^*$.  In fact, they are \emph{also} maps that take a vector and a row vector into a number:
\begin{align}
    M:\; V\otimes V^* \to \# \ .
\end{align}
The left-hand side just says ``$M$ is a map that takes...''
We have introduced the $\otimes$ \textbf{tensor product}\index{tensor product} notation.\sidenote{I'll be real honest with you: sometimes I just write this as $\times$.} The tensor product is like a `multiplication' of \emph{spaces}. For now we leave the definition implicit---the above line simply means a map that takes an element of $V$ and an element of $V^*$ to produce a number. We further qualify what kind of map this must be shortly.\sidenote{Spoiler: it has to be linear.}

This means you can think of row vectors as maps from vectors to numbers, and vectors are a map that take row vectors to numbers:
\begin{align}
    \row{w}:&\; V\to \# &
    \vec{v}:&\; V^* \to \# \ .
\end{align}
...
\begin{exercise}
What are all of the possible ways of treating a $(p,q)$ tensor as a map? 
\end{exercise}
